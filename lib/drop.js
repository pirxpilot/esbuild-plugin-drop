...
function drop(src, modules) {
  const sourceType = /^\s*(export|import)\b/m.test(src) ? 'module' : 'script';
  ...
  function removeReferences(binding) {
    binding.getReferences().forEach(ref => {
      ...
      if (use.type === 'CallExpression') {
        ...
        binding.remove(ref);
        if (use.parent.type === 'VariableDeclarator') {
          // need also remove all references to declared variable
          const binding = getBinding(use.parent.id);
          removeReferences(binding);
          if (!binding.isReferenced()) {
            removeDeclaration(use);
          }
        }
      }
    });
